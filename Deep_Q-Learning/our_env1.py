...
class dynetworkEnv(gym.Env):
        ...
        for nodeIdx in self.dynetwork._network.nodes:
            ...
            for i in range(queue_size):
                ...
                if SP:
                    ...
                    action = random.choice(list(self.dynetwork._network.neighbors(pkt_state[0])))
                else:
    ...
    def draw(self, i, currTrial):
        ...
        nx.draw_networkx_labels(self.dynetwork._network.nodes, pos=self._positions,labels=node_labels,font_size=8,font_color='k')
        if self.print_edge_weights:
            edge_labels = nx.get_edge_attributes(
                self.dynetwork._network, 'edge_delay')
    ...
    def get_next_step(self, currPos, destPos, router_type):
        # else:
            # print(nx.dijkstra_path(self.dynetwork._network, currPos, destPos, weight='edge_delay'))
